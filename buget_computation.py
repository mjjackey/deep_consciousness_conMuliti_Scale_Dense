...
def dynamic_evaluate(model, test_loader, val_loader, args):
    ...
    print("val_pred(logits):",val_pred.size(),"n_stage:",val_pred.size(0))

    # flops = torch.load(os.path.join(args.savedir, 'flops.pth'))   #######

    flops_list=[16263680.0
                ,22849802.0
                ,33842708.0
                ,51233054.0
                ,77257512.0
                ,102373938.0
                ,128977980.0]
    flops = torch.FloatTensor(flops_list)

    with open(os.path.join(args.savedir, 'dynamic.txt'), 'w') as fout:
        for p in range(1, 40):
            print("*********************")
            _p = torch.FloatTensor(1).fill_(p * 1.0 / 20)
            probs = torch.exp(torch.log(_p) * torch.range(1, args.msd_blocks))
            probs /= probs.sum()
        ...
        for i in range(n_sample):
            ...
            for k in range(n_stage):
                if max_preds[k][i].item() >= T[k]: # force the sample to exit at k
                    ...
                    break
        acc_all = 0
        for k in range(n_stage):  ######## the dimension of flops is n_stage
