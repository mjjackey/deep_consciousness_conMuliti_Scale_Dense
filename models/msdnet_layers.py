...
"""
Module: Base class for all neural network modules. Your models should also subclass this class.
Modules can also contain other Modules, allowing to nest them in a tree structure.
Sequential: Modules will be added to it in the order they are passed in the constructor. Sequential can contain
Module
ModuleLists: Holds submodules in a list, 
"""
class _DynamicInputDenseBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for calc, m in enumerate(self.conv_modules):
            out = torch.cat([out, m(x[calc + 1])], 1)  # m(x[calc + 1])?
...
class MSDLayer(nn.Module):
    ...
    '''
    scales 2 以后每个节点的同层的水平连接和上层的对角连接
    '''
    def build_down_densenet(self, in_channels1, in_channels2, out_channels,
        ...
        conv_module1 = self.convolve(in_channels1, int(out_channels/2), 'down',
                                    bottleneck, bn_width1)  # the same layer-regular conv
        conv_module2 = self.convolve(in_channels2, int(out_channels/2), 'normal',
                                    bottleneck, bn_width2)  # out_channels/2 ?, the up layer, strided conv
    ...
    def convolve(self, in_channels, out_channels, conv_type,
        ...
        if bottleneck:
            tmp_channels = int(min([in_channels, bn_width * out_channels])) # bn_width * out_channels?
    ...
    def forward(self, x):
        ...
        # Prepare the different scales' inputs of the current transition/regular layer
        if self.to_drop: # non-zero # Transition
            ...
            for scale in range(1, self.out_scales):
                last_same_scale = x[scale]   # 下标 1~out_scales
                last_upper_scale = x[scale - 1]  # subscript 0 - out_scales-1
                cur_input.append([last_same_scale,
                                  last_upper_scale,
                                  last_same_scale])
        ...
        for scale in range(0, self.out_scales):
            outputs.append(self.subnets[scale](cur_input[scale])) # cur_input[scale]
        return outputs
